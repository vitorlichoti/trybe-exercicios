...
app.delete('/movies/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const movies = writeFile();
    const filteredMovie = movies.filter((e) => e.id !== Number(id));
    const updateMovies = JSON.stringify(filteredMovie, null, 2);
    await fs.writeFile(moviesPath, updateMovies);
    res.status(204).end();
  } catch (error) {
    res.status(500).send({ message: error.message });
...
// bonuss
app.get('/movies/seach', async (req, res) => {
  try { 
    const { q } = req.query;
    const movies = await readFile();
